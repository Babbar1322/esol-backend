    ...
    <div class="card-body p-3">
        <div class="row">
            <div class="col-md-6">
                ...
                <h5>Submitted On: {{ \Carbon\Carbon::createFromTimestampMs($user_test->submit_time)->toDateTimeString() }}</h5>
            </div>
            <div class="col-md-6">
                <h5>Groups</h5>
                <ul>
        ...
        <div>
            @foreach ($test->test_groups as $group)
            <div class="h4">{{$group->group_name}}</div>
            {!! $group->group_content !!}
            <textarea class="form-control" rows="10" disabled>{{$group->submitted_answer}}</textarea>
            <div class="row my-2">
                <div class="col">
                    <button class="btn btn-success rounded-pill" {{$group->is_checked ? 'disabled' : null}} onclick="validate({{$group->submitted_id}}); disableButtons(this)">Validate</button>
                </div>
                <div class="col">
                    <button class="btn btn-danger rounded-pill" {{$group->is_checked ? 'disabled' : null}} onclick="invalidate({{$group->submitted_id}}); disableButtons(this)">Invalidate</button>
                </div>
            </div>
            @endforeach
        </div>
